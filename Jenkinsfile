pipeline {
    ...
    environment {
        ...
        //IMAGE_NAME = "wimetrixcregistery/Nishat-backend-order"
        GIT_CREDENTIALS_ID = "GithubCredentials"
        SOURCE_REPO = "github.com/WiMetrixDev/sooperwizer.git"
        WORKSPACE_DIR = "/home/jenkins/deployment-package/wimetrix/Nishat-backend-order"
        GITHUB_REPO = 'github.com/WiMetrixDev/dep-Nishat-backend-order.git'
    ...
    stages {
            ...
            steps {
                script
                {
                    ...
                    if (params.DEP_BRANCH == 'main') {
                        env.IMAGE_NAME = "wimetrixcregistery/Nishat-backend-order"
                    } 
                    
                    else if (params.DEP_BRANCH == 'qa') {
                        env.IMAGE_NAME = "wimetrixcregistery/Nishat-backend-orderqa"
                    } 
        ...
        steps {
            ...
            checkout([$class: 'GitSCM',
                    ...
                    extensions: [
                        [$class: 'WipeWorkspace'],
                        [$class: 'RelativeTargetDirectory', relativeTargetDir: '/home/jenkins/deployment-package/wimetrix/Nishat-backend-order']
                    ],
                    submoduleCfg: [],
                    userRemoteConfigs: [[
                        credentialsId: 'GithubCredentials',
                        url: 'https://github.com/WiMetrixDev/dep-Nishat-backend-order.git'
                    ]]
            ])
            ...
            checkout([$class: 'GitSCM',
                    ...
                    extensions: [
                        [$class: 'WipeWorkspace'],
                        [$class: 'RelativeTargetDirectory', relativeTargetDir: '/home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source']
                    ],
                    submoduleCfg: [],
                    userRemoteConfigs: [[
                        credentialsId: 'GithubCredentials',
    ...
    steps {
        script {
            def dockerfileDir = '/home/jenkins/deployment-package/wimetrix/Nishat-backend-order/'
            def sourceDir = "${dockerfileDir}source"
            withCredentials([
                        file(credentialsId: 'NISHAT_BACKEND_ORDER_ENV_PROD', variable: 'SECRET_FILE')
                    ]){

                    sh label: '', script: '''
                        cp /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/Dockerfile /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source/
                        cp /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/.dockerignore /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source/
                        cd /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source/shared-env/backend
                        cp ${SECRET_FILE} /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source/shared-env/backend/.env.production
                '''
                dockerImage = docker.build("${REGISTRY_URL}/${IMAGE_NAME}:${env.BUILD_ID}",
                    "-f ${sourceDir}/Dockerfile ${sourceDir}")

                sh label: '', script: '''
                rm -rf /home/jenkins/deployment-package/wimetrix/Nishat-backend-order/source/*
                ''' 
                    }                 
            }
        }
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'GITHUB_ACTIONS_SECRET', variable: 'GITHUB_ACTIONS')]){
                        ...
                        def githubRepo = "dep-Nishat-backend-order"  
